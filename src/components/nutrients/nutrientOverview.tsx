...
export const NutrientOverview = ({ foodOffers, setModalOpen }) => {
	const notSoldOutFoodOffers = useMemo(
		() => foodOffers.filter((foodOffer) => foodOffer.sold_out === false),
		[foodOffers]
	)
	...
	const getShortFoodName = (index: number) => {
		...
		foodTitleTrim = foodTitleTrim.substr(
			0,
			Math.min(foodTitleTrim.length, foodTitleTrim.lastIndexOf(" "))
		)
		//if comma is at the end, remove
		if (foodTitleTrim[foodTitleTrim.length - 1] === ",")
			foodTitleTrim = foodTitleTrim.substring(0, foodTitleTrim.length - 1)
		//add ellipsis
		foodTitleTrim += "…"
	...
	return (
		<div className="h-screen bg-light-green">
			<div className="m-auto flex h-full max-w-lg flex-col space-y-6 overflow-scroll">
				<div className=" border-b border-gray/20">
					...
					>
						<img
							...
							className="mr-auto w-4 rotate-180"
						/>
						<h2 className="font-sans-bold">Nährwerte</h2>
				...
				{notSoldOutFoodOffers.map((foodOffer, index) => {
					...
					if (index % 2 === 0) {
						return (
							<div
								key={index}
								className="flex flex-col divide-y divide-gray/20 border-b border-gray/20 first:border-0"
							>
								<div className="flex w-full flex-row space-x-4 px-4 py-2 font-serif-med text-sm">
									<p className="w-3/12 grow-0"></p>
									<p className="w-3/12 grow">
										{getShortFoodName(index)}
									</p>

									{twoAvailable ? (
										<p className="w-3/12 grow">
											{getShortFoodName(index + 1)}
										</p>
									) : (
										<div className="w-3/12 grow"></div>
									)}
								</div>
								{notSoldOutFoodOffers[index].nutrients.map(
									(nutrient, i) => {
										return (
											<div
												className="flex w-full flex-row items-center justify-between space-x-4 px-4 py-3 text-sm"
												key={`${index}-nutrient-${i}`}
											>
												<p className="w-3/12 grow-0 break-words font-sans-med text-gray/50">
													{nutrientType[
														notSoldOutFoodOffers[
															index
														].nutrients[i].name
													]?.name || nutrient.name}
												</p>
				...
				<p className="px-4 font-sans-reg text-gray/50">
					Verglichen mit dem Tagesbedarf <br /> für durchschnittliche
					Person lt. DGE
				</p>
			</div>
		</div>
