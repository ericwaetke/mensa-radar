...
export const getServerSideProps: GetServerSideProps = async (context) => {
	const meal: FoodOffering = await supabase
		.from("food_offerings")
		.select("*")
		.eq("id", context.params.mealId)
		.single()
		.then((res) => res.data as FoodOffering)
	return {
		props: {
			meal,
		},
...
export default function SharedMeal({ meal }: { meal: FoodOffering }) {
	...
	const { mensa, day } =
		router.query !== undefined
			? router.query
			: { mensa: "Mensa not Found", day: "freitag" }

	useEffect(() => {
		router.push(`/mensa/${mensa}/${day}#${meal.id}`)
	}, [])

	const foodType = meal.vegetarian
		? "Vegetarisch"
		: meal.vegan
		? "Vegan"
		: meal.fish
	...
	return (
		<>
			<Head>
				...
				<meta
					property="og:url"
					content={`https://mensa-radar.de/mensa/${mensa}/${day}`}
				/>{" "}
				:
				<meta
				...
				<meta
					...
					content={`${foodType} - Essen vom ${meal.date} in der Mensa ${meal.mensa}!`}
				/>
				<meta
